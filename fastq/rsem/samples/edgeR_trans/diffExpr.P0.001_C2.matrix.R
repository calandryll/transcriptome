...
if (file.exists("diffExpr.P0.001_C2.matrix.RData")) {
    print('RESTORING DATA FROM EARLIER ANALYSIS')
    load("diffExpr.P0.001_C2.matrix.RData")
} else {
    print('Reading matrix file.')
    primary_data = read.table("diffExpr.P0.001_C2.matrix", header=T, com='', sep="\t", row.names=1, check.names=F)
    primary_data = as.matrix(primary_data)
}
source("/home/chris/bin/trinityrnaseq/Analysis/DifferentialExpression/R/heatmap.3.R")
source("/home/chris/bin/trinityrnaseq/Analysis/DifferentialExpression/R/misc_rnaseq_funcs.R")
source("/home/chris/bin/trinityrnaseq/Analysis/DifferentialExpression/R/pairs3.R")
data = primary_data
samples_data = read.table("samples_described.txt", header=F, check.names=F)
...
write.table(data, file="diffExpr.P0.001_C2.matrix.log2.dat", quote=F, sep='	');
sample_cor = cor(data, method='pearson', use='pairwise.complete.obs')
sample_dist = dist(t(data), method='euclidean')
hc_samples = hclust(sample_dist, method='complete')
pdf("diffExpr.P0.001_C2.matrix.log2.sample_cor_matrix.pdf")
...
", "diffExpr.P0.001_C2.matrix.log2") , ColSideColors=sampleAnnotations, RowSideColors=t(sampleAnnotations))
dev.off()
gene_cor = NULL
